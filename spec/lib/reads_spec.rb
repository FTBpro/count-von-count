...
describe "Read Action" do
  before :all do
    ...
    league_id = rand(10)
    team_id = rand(20)
    @league_weekly = create :LeagueWeekly, { league: league_id, week: Time.now.strftime("%W"), year: Time.now.strftime("%Y") }
    @league_monthly = create :LeagueMonthly, { league: league_id, month: Time.now.strftime("%m"), year: Time.now.strftime("%Y") }
    @team_weekly = create :TeamWeekly, { team: team_id, week: Time.now.strftime("%W"), year: Time.now.strftime("%Y") }
    @team_monthly = create :TeamMonthly, { team: team_id, month: Time.now.strftime("%m"), year: Time.now.strftime("%Y") }
  end
  ...
  before :all do
    open("http://#{HOST}/reads?post=#{@post.id}&user=#{@user.id}&author=#{@author.id}&league=#{@league_weekly.ids[:league]}&team=#{@team_weekly.ids[:team]}")
  end
  ...
  describe "Team/League Weekly/Monthly" do
    ...
    it "should increase the counts for the same team objects as above, although the other params have changed" do
      open("http://#{HOST}/reads?post=#{@post.id}&user=#{rand(100)}&author=#{rand(100)}&league=#{rand(20)}&team=#{@team_weekly.ids[:team]}")
