...
describe "Login" do
  ...
  before :all do
    open("http://#{HOST}/feature?user=#{@user.id}&locale=#{@locale}&category=#{@category}&" + @channels.map { |c| "channel[]=#{c}"}.join("&") )
  ...
  describe "User" do
    ...
    it "should increase the user's num of featured posts according to the given channel" do
      open("http://#{HOST}/feature?user=#{@user.id}&locale=#{@locale}&category=funny&channel=#{@other_channel}")
  ...
  describe "CategoryMonthlyFeatured" do
    ...
    it "should not count if there is no category" do
      ...
      open("http://#{HOST}/feature?user=#{@user.id}&locale=#{@locale}&category=&channel=#{@other_channel}")
