...
describe "Post Created" do
  before :all do
    ...
    @league_posts = create :LeaguePosts, { id: @league_id }
  ...
  describe "LeaguePosts" do
    it "should increase the league posts count if league_count is not false" do
      @league_posts.data["posts"].to_i.should eq @league_posts.initial_data["posts"].to_i + 1
    end
  ...
  describe "LeagueWriters" do
    it "should add 1 to the user who created the post" do
      @league_writers.set["user_#{@user.id}"].to_i.should == @league_writers.initial_set["user_#{@user.id}"].to_i + 2  # greater by 2 because of previous spec who does another call
    end
